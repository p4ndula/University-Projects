...
class SecurityController extends Controller
{
    ...
    public function actionLogin()
    {
        ...
        if ($model->load(\Yii::$app->getRequest()->post()) && $model->validate()) {
            $user = User::findOne(['username' => $model->login]);
            /**
             * Before login, check if the user has 'admin' role assigned
             * Return error if not authorized
             */
            if (!empty($user) && (
                    Yii::$app->authManager->checkAccess($user->id, 'admin') ||
                    Yii::$app->authManager->checkAccess($user->id, 'manager') ||
                    Yii::$app->authManager->checkAccess($user->id, 'operator') ||
                    $user->isAdmin)) {
                ...
                $model->addError('login', 'This user is not authorized for administration');
            }
        }
